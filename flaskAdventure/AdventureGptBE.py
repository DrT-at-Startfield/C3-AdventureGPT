...
class AdventureGptBE:
    ...
    def game_intro(self):
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": self.game_structure},
                {"role": "system", "content": self.base_prompt},
                {"role": "user", "content": "Describe the premise of the game."},
            ]
        )
    ...
    def strip_choices_from_result(self, result):
        ...
        choices = []
        choices.append(result[offset + result[offset:].find('1. '):offset + result[offset:].find('2. ')].strip())
        choices.append(result[offset + result[offset:].find('2. '):offset + result[offset:].find('3. ')].strip())
        choices.append(result[offset + result[offset:].find('3. '):offset + result[offset:].find('4. ')].strip())
    ...
    def get_choices_from_chatbot(self, last_result):
        response = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "Give me a set of 4 options what I can do next"}
            ]
        )
    ...
    def build_message(self, choice, last_result):
        ...
        if len(choice) == 0:
            ...
            message.append({"role": "user", "content": "Describe what happens when I {0}.".format(choice)})
        return message
